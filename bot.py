...
async def handle_plain_message(message):
    ...
    # Save message to DB (skip audit channels)
    AUDIT_CHANNELS = ["role-delete", "role-create", "role-update", "member-ban",
    ...
    if message.channel.name not in AUDIT_CHANNELS:
        try:
            conn = sqlite3.connect("messages.db")
            c = conn.cursor()
            c.execute("""INSERT OR IGNORE INTO messages
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (str(message.id), str(message.guild.id), message.channel.name,
                 message.author.name, str(message.author.id), message.content,
